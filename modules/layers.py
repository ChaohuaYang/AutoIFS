...
class FeatSelectLayer(torch.nn.Module):
    def __init__(self, field_size, hidden_dim, stop_gradient=True):
        super().__init__()
        self.stop_gradient = stop_gradient
        self.se_mlp_1 = torch.nn.Linear(field_size, hidden_dim)
        self.se_mlp_2 = torch.nn.Linear(hidden_dim, field_size)

    def forward(self, inputs):
        emb_sg = inputs.detach() if self.stop_gradient else inputs  # Detach to stop gradient flow

        emb_pool = torch.mean(emb_sg, dim=-1)  # Pooling across the embedding dimension
        se_info = F.relu(self.se_mlp_1(emb_pool))  # Applying first SE layer
        se_info = self.se_mlp_2(se_info)  # Applying second SE layer
        se_info = se_info.unsqueeze(2)  # Expanding the dimensions to match inputs
        emb_se = 2 * torch.sigmoid(se_info) * inputs
        return emb_se